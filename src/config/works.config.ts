import tulip1 from '@img/works/tulips/01.webp';
import tulip2 from '@img/works/tulips/02.webp';
import tulip3 from '@img/works/tulips/03.webp';
import tulip4 from '@img/works/tulips/04.webp';
import tulip5 from '@img/works/tulips/05.webp';
import tulip6 from '@img/works/tulips/06.webp';

import hydrangea1 from '@img/works/hydrangeas/01.webp';
import hydrangea2 from '@img/works/hydrangeas/02.webp';
import hydrangea3 from '@img/works/hydrangeas/03.webp';
import hydrangea4 from '@img/works/hydrangeas/04.webp';
import hydrangea5 from '@img/works/hydrangeas/05.webp';
import hydrangea6 from '@img/works/hydrangeas/06.webp';
import hydrangea7 from '@img/works/hydrangeas/07.webp';

import seedling1 from '@img/works/seedling/01.webp';
import seedling2 from '@img/works/seedling/02.webp';
import seedling3 from '@img/works/seedling/03.webp';
import seedling4 from '@img/works/seedling/04.webp';
import seedling5 from '@img/works/seedling/05.webp';
import seedling6 from '@img/works/seedling/06.webp';
import seedling7 from '@img/works/seedling/07.webp';

import conifers1 from '@img/works/conifers/01.webp';
import conifers2 from '@img/works/conifers/02.webp';
import conifers3 from '@img/works/conifers/03.webp';
import conifers4 from '@img/works/conifers/04.webp';
import conifers5 from '@img/works/conifers/05.webp';
import conifers6 from '@img/works/conifers/06.webp';

import strawberry1 from '@img/works/strawberry/01.webp';
import strawberry2 from '@img/works/strawberry/02.webp';

// Shrubs - 11 webp файлов (02-12) Лиственные
import shrubs1 from '@img/works/shrubs/01.webp';
import shrubs2 from '@img/works/shrubs/02.webp';
import shrubs3 from '@img/works/shrubs/03.webp';
import shrubs4 from '@img/works/shrubs/04.webp';
import shrubs5 from '@img/works/shrubs/05.webp';

const worksConfig = {
    tabs: [
        {
            id: 'tulips',
            text: 'Тюльпаны',
        },
        {
            id: 'hydrangeas',
            text: 'Гортензии',
        },
        {
            id: 'seedling',
            text: 'Рассада для цветов',
        },
        {
            id: 'conifers',
            text: 'Хвойные растения',
        },
        {
            id: 'shrubs',
            text: 'Лиственные кустарники',
        },
        {
            id: 'strawberry',
            text: 'Рассада клубники',
        },
    ],
    items: {
        tulips: [
            {
                id: 'wt1',
                slideId: 0,
                img: tulip1,
            },
            {
                id: 'wt2',
                slideId: 1,
                img: tulip2,
            },

            {
                id: 'wt3',
                slideId: 2,
                img: tulip3,
            },
            {
                id: 'wt4',
                slideId: 3,
                img: tulip4,
            },
            {
                id: 'wt5',
                slideId: 4,
                img: tulip5,
            },

            {
                id: 'wt6',
                slideId: 5,
                img: tulip6,
            },
        ],
        hydrangeas: [
            {
                id: 'wh1',
                slideId: 0,
                img: hydrangea1,
            },
            {
                id: 'wh2',
                slideId: 1,
                img: hydrangea2,
            },
            {
                id: 'wh3',
                slideId: 2,
                img: hydrangea3,
            },
            {
                id: 'wh4',
                slideId: 3,
                img: hydrangea4,
            },
            {
                id: 'wh5',
                slideId: 4,
                img: hydrangea5,
            },
            {
                id: 'wh6',
                slideId: 5,
                img: hydrangea6,
            },
            {
                id: 'wh7',
                slideId: 6,
                img: hydrangea7,
            },
        ],
        seedling: [
            {
                id: 'ws1',
                slideId: 0,
                img: seedling1,
            },
            {
                id: 'ws2',
                slideId: 1,
                img: seedling2,
            },
            {
                id: 'ws3',
                slideId: 2,
                img: seedling3,
            },
            {
                id: 'ws4',
                slideId: 3,
                img: seedling4,
            },
            {
                id: 'ws5',
                slideId: 4,
                img: seedling5,
            },
            {
                id: 'ws6',
                slideId: 5,
                img: seedling6,
            },
            {
                id: 'ws7',
                slideId: 6,
                img: seedling7,
            },
        ],
        conifers: [
            {
                id: 'wcf1',
                slideId: 0,
                img: conifers1,
            },
            {
                id: 'wcf2',
                slideId: 1,
                img: conifers2,
            },
            {
                id: 'wcf3',
                slideId: 2,
                img: conifers3,
            },
            {
                id: 'wcf4',
                slideId: 3,
                img: conifers4,
            },
            {
                id: 'wcf5',
                slideId: 4,
                img: conifers5,
            },
            {
                id: 'wcf6',
                slideId: 5,
                img: conifers6,
            },
        ],
        shrubs: [
            {
                id: 'wsh1',
                slideId: 0,
                img: shrubs1,
            },
            {
                id: 'wsh2',
                slideId: 1,
                img: shrubs2,
            },
            {
                id: 'wsh3',
                slideId: 2,
                img: shrubs3,
            },
            {
                id: 'wsh4',
                slideId: 3,
                img: shrubs4,
            },
            {
                id: 'wsh5',
                slideId: 4,
                img: shrubs5,
            },
        ],
        strawberry: [
            {
                id: 'wst1',
                slideId: 0,
                img: strawberry1,
            },
            {
                id: 'wst2',
                slideId: 1,
                img: strawberry2,
            },
        ],
    },
};
export default worksConfig;
